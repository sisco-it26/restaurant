          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? 'text-moss-600' : 'text-stone-600 hover:text-moss-600'
              )}
            >
              <Icon className="w-5 h-5" />
              <span className="text-xs font-medium">{item.label}</span>
              {isCart && itemCount > 0 && (
                <span className="absolute top-2 right-1/4 bg-salmon-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">
                  {itemCount}
                </span>
              )}
            </Link>
